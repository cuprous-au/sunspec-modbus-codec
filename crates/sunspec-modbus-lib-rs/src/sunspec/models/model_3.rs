...
pub const SIZE: u16 = 61;

static POINTS: [PointDetails<()>; 60] = [
    PointDetails {
        point: |()| Point::ModelId,
        size: 1,
        start_address: 0,
...
pub fn write_point<'a>(
    ...
    match point {
        ...
        Point::Off41 => {
            buffer::write_u16(model.off41(), buffer);
        }
        Point::Off42 => {
            buffer::write_u16(model.off42(), buffer);
        }
        Point::Off43 => {
            buffer::write_u16(model.off43(), buffer);
        }
        Point::Off44 => {
            buffer::write_u16(model.off44(), buffer);
        }
        Point::Off45 => {
            buffer::write_u16(model.off45(), buffer);
        }
        Point::Off46 => {
            buffer::write_u16(model.off46(), buffer);
        }
        Point::Off47 => {
            buffer::write_u16(model.off47(), buffer);
        }
        Point::Off48 => {
            buffer::write_u16(model.off48(), buffer);
        }
        Point::Off49 => {
            buffer::write_u16(model.off49(), buffer);
        }
        Point::Off50 => {
            buffer::write_u16(model.off50(), buffer);
        }
        Point::Timestamp => {
