...
use sunspec_modbus_lib_rs::sunspec::models::model_710::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model710CallbackAdapter`, valid for the
...
unsafe fn model_710_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model710CallbackAdapter).as_ref() };
...
unsafe fn model_710_c_visit_write(
    ...
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model710CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_710_callback(
    adapter: *mut Model710CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_710,
