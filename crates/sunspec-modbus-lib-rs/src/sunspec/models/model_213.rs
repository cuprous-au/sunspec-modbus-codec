...
impl ReadAdapter for Model213CallbackAdapter {
    fn amps(&self) -> f32 {
...
impl ReadAdapter for Model213StatefulAdapter {
    fn amps(&self) -> f32 {
        self.amps
    }
...
/// C-FFI dispatch descriptor for SunSpec model 213. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_213: StaticModelSpec = StaticModelSpec {
    id: 213,
    length: model_213_c_length,
    writable: false,
    visit_read: model_213_c_visit_read,
    visit_write: model_213_c_visit_write,
};

fn model_213_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model213).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model213{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_213_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model213StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model213CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_213_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_213_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
