...
pub static POINTS: [ReadablePoint; 58] = [
    ...
    ReadablePoint {
        reference: PointReference::Model802 { point: Point::Operation },
        size: 1,
        data_type: PointType::Enum16,
        writeable: true,
    },
    ReadablePoint {
        reference: PointReference::Model802 { point: Point::InverterState },
        size: 1,
        data_type: PointType::Enum16,
        writeable: true,
    },
...
pub fn write_point(model: &dyn ModelAdapter, point: &Point, buffer: &mut [u16], offset: u16, limit: u16) {
    match point {
        ...
        Point::StateOfCharge => {
            serialisation::write_u16(model.state_of_charge(), buffer);
        },
        ...
        Point::ControlMode => {
            serialisation::write_u16(model.control_mode() as u16, buffer);
        },
        ...
        Point::AlarmReset => {
            serialisation::write_u16(model.alarm_reset(), buffer);
        },
        Point::BatteryType => {
            serialisation::write_u16(model.battery_type() as u16, buffer);
        },
        ...
        Point::TotalDcCurrent => {
            serialisation::write_i16(model.total_dc_current(), buffer);
        },
        ...
        Point::TotalPower => {
            serialisation::write_i16(model.total_power(), buffer);
        },
        ...
        Point::Operation => {
            serialisation::write_u16(model.operation() as u16, buffer);
        },
        Point::InverterState => {
            serialisation::write_u16(model.inverter_state() as u16, buffer);
        },
        Point::AhRtgSf => {
            serialisation::write_u16(model.ah_rtg_sf(), buffer);
        },
        Point::WhRtgSf => {
            serialisation::write_u16(model.wh_rtg_sf(), buffer);
        },
        Point::WChaDisChaMaxSf => {
            serialisation::write_u16(model.w_cha_dis_cha_max_sf(), buffer);
        },
        Point::DisChaRteSf => {
        ...
        Point::AMaxSf => {
            serialisation::write_u16(model.a_max_sf(), buffer);
        },
        Point::WSf => {
...
impl ModelAdapter for Model802CallbackAdapter {
    ...
    /// Self discharge rate. Percentage of capacity (WHRtg) discharged per day.
    fn self_discharge_rate(&self) -> Option<u16> {
        self.self_discharge_rate_callback.map(|callback| {
    ...
    /// Manufacturer maximum state of charge, expressed as a percentage.
    fn nameplate_max_so_c(&self) -> Option<u16> {
        self.nameplate_max_so_c_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Manufacturer minimum state of charge, expressed as a percentage.
    fn nameplate_min_so_c(&self) -> Option<u16> {
        self.nameplate_min_so_c_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Setpoint for maximum reserve for storage as a percentage of the nominal maximum storage.
    fn max_reserve_percent(&self) -> Option<u16> {
        self.max_reserve_percent_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Setpoint for minimum reserve for storage as a percentage of the nominal maximum storage.
    fn min_reserve_percent(&self) -> Option<u16> {
        self.min_reserve_percent_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Measurement.
    fn depth_of_discharge(&self) -> Option<u16> {
        self.depth_of_discharge_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Percentage of battery life remaining.
    fn state_of_health(&self) -> Option<u16> {
        self.state_of_health_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Number of cycles executed in the battery.
    fn cycle_count(&self) -> Option<u32> {
        self.cycle_count_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Charge status of storage device. Enumeration.
    fn charge_status(&self) -> Option<ChaSt> {
        self.charge_status_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Value is incremented every second with periodic resets to zero.
    fn battery_heartbeat(&self) -> Option<u16> {
        self.battery_heartbeat_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Vendor specific battery bank state. Enumeration.
    fn vendor_battery_bank_state(&self) -> Option<u16> {
        self.vendor_battery_bank_state_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Number of days since 1/1/2000.
    fn warranty_date(&self) -> Option<u32> {
        self.warranty_date_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// If not implemented, must implement AChaMax and ADisChaMax.
    fn max_battery_voltage(&self) -> Option<u16> {
        self.max_battery_voltage_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// If not implemented, must implement AChaMax and ADisChaMax.
    fn min_battery_voltage(&self) -> Option<u16> {
        self.min_battery_voltage_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// String containing the cell with maximum voltage.
    fn max_cell_voltage_string(&self) -> Option<u16> {
        self.max_cell_voltage_string_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// String containing the cell with minimum voltage.
    fn min_cell_voltage_string(&self) -> Option<u16> {
        self.min_cell_voltage_string_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Calculation which is always unsigned (i.e. magnitude only). If not implemented, must implement VMax and VMin.
    fn max_charge_current(&self) -> Option<u16> {
        self.max_charge_current_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Calculation which is always unsigned (i.e. magnitude only). If not implemented, must implement VMax and VMin.
    fn max_discharge_current(&self) -> Option<u16> {
        self.max_discharge_current_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Used in special states such as manual battery charging.
    fn inverter_state_request(&self) -> Option<ReqInvState> {
        self.inverter_state_request_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Used in special states such as string balancing.
    fn battery_power_request(&self) -> Option<i16> {
        self.battery_power_request_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Scale factor for self discharge rate.
    fn dis_cha_rte_sf(&self) -> Option<u16> {
        self.dis_cha_rte_sf_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Scale factor for AC power request.
    fn w_sf(&self) -> Option<u16> {
        self.w_sf_callback.map(|callback| {
        (callback)(self.context)
        })
    }
}
...
impl ModelAdapter for Model802StatefulAdapter {
    ...
    fn vendor_battery_bank_state(&self) -> Option<u16> {
        Some(
        self.vendor_battery_bank_state
        )
    }
