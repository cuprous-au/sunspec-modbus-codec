...
use sunspec_modbus_lib_rs::sunspec::models::model_707::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model707CallbackAdapter`, valid for the
...
unsafe fn model_707_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model707CallbackAdapter).as_ref() };
...
unsafe fn model_707_c_visit_write(
    ...
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model707CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_707_callback(
    adapter: *mut Model707CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_707,
