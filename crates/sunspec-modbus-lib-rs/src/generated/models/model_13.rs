...
impl ReadAdapter for Model13StatefulAdapter {
    fn name(&self) -> Option<&CStr> {
...
impl WriteAdapter for Model13StatefulAdapter {
    /// Name (Nam)
    ///
    /// Interface name
    fn set_name(&mut self, value: &CStr) {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model13{Stateful,Callback}Adapter`,
...
unsafe fn model_13_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model13StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model13CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_13_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model13StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model13CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
