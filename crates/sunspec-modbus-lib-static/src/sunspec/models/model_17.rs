...
use sunspec_modbus_lib_rs::sunspec::models::model_17::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model17CallbackAdapter`, valid for the
...
unsafe fn model_17_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model17CallbackAdapter).as_ref() };
...
unsafe fn model_17_c_visit_write(
    ...
    let model = Model17;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model17CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_17_callback(
    adapter: *mut Model17CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_17,
