...
/// Marker for SunSpec model 138. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model138;

impl<'ad> ModelSpec<'ad> for Model138 {
    const MODEL_ID: u16 = 138;
...
#[repr(u16)]
pub enum Model138ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model138ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model138ReadOnly> {
        match repr {
            0 => Some(Model138ReadOnly::Readwrite),
            1 => Some(Model138ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model138ReadOnly`] - only `Model138ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model138ReadOnly;
...
pub struct Model138CallbackAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    curve_read_only_callback: extern "C" fn(*const c_void) -> Model138ReadOnly,
}

impl ReadAdapter for Model138CallbackAdapter {
    fn act_crv(&self) -> u16 {
...
impl WriteAdapter for Model138CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
pub struct Model138StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model138ReadOnly,
}

impl ReadAdapter for Model138StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model138StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 138. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_138: StaticModelSpec = StaticModelSpec {
    id: 138,
    length: model_138_c_length,
    writable: true,
    visit_read: model_138_c_visit_read,
    visit_write: model_138_c_visit_write,
};

fn model_138_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model138).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model138{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_138_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model138StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model138CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_138_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_138_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model138StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model138CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
