...
use sunspec_modbus_lib_rs::sunspec::models::model_130::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model130CallbackAdapter`, valid for the
...
unsafe fn model_130_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model130CallbackAdapter).as_ref() };
...
unsafe fn model_130_c_visit_write(
    ...
    let model = Model130;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model130CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_130_callback(
    adapter: *mut Model130CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_130,
