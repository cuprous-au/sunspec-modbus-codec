...
use sunspec_modbus_lib_rs::sunspec::models::model_139::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model139CallbackAdapter`, valid for the
...
unsafe fn model_139_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model139CallbackAdapter).as_ref() };
...
unsafe fn model_139_c_visit_write(
    ...
    let model = Model139;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model139CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_139_callback(
    adapter: *mut Model139CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_139,
