...
/// Marker for SunSpec model 709. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model709 {
...
impl<'ad> ModelSpec<'ad> for Model709 {
    const MODEL_ID: u16 = 709;
...
pub(crate) fn read_point_from_buffer<'a>(
    ...
    match point {
        Point::DerTripLfModuleEnable => {
            ...
            adapter.set_der_trip_lf_module_enable(enum_value);
            Ok(())
        }
        Point::AdoptCurveRequest => {
            let value = buffer.read_u16()?;
...
impl Model709AdptCrvRslt {
    pub fn from_repr(repr: u16) -> Option<Model709AdptCrvRslt> {
        match repr {
            0 => Some(Model709AdptCrvRslt::InProgress),
            1 => Some(Model709AdptCrvRslt::Completed),
            2 => Some(Model709AdptCrvRslt::Failed),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model709AdptCrvRslt`] - only `Model709AdptCrvRslt` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `AdptCrvRslt` distinct from another model's point of the same name.
pub type AdptCrvRslt = Model709AdptCrvRslt;

#[derive(Clone, Copy, Debug, PartialEq)]
#[repr(u16)]
pub enum Model709Ena {
    /// Disabled
    ///
...
impl Model709Ena {
    pub fn from_repr(repr: u16) -> Option<Model709Ena> {
        match repr {
            0 => Some(Model709Ena::Disabled),
            1 => Some(Model709Ena::Enabled),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model709Ena`] - only `Model709Ena` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `Ena` distinct from another model's point of the same name.
pub type Ena = Model709Ena;

#[derive(Clone, Copy, Debug, PartialEq)]
#[repr(u16)]
pub enum Model709ReadOnly {
    /// Read-Write Access
    ///
...
impl Model709ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model709ReadOnly> {
        match repr {
            0 => Some(Model709ReadOnly::Rw),
            1 => Some(Model709ReadOnly::R),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model709ReadOnly`] - only `Model709ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model709ReadOnly;
...
pub struct Model709CallbackAdapter {
    ...
    ///
    /// DER low frequency trip control enable.
    der_trip_lf_module_enable_callback: extern "C" fn(*const c_void) -> Model709Ena,
    /// DER Trip LF Module Enable (Ena)
    ///
    /// DER low frequency trip control enable.
    set_der_trip_lf_module_enable_callback: extern "C" fn(Model709Ena, *mut c_void),
    /// Adopt Curve Request (AdptCrvReq)
    ///
    /// Index of curve points to adopt. First curve index is 1.
    ...
    ///
    /// Result of last adopt curve operation.
    adopt_curve_result_callback: extern "C" fn(*const c_void) -> Model709AdptCrvRslt,
    /// Number Of Points (NPt)
    ///
    /// Number of curve points supported.
    ...
    ///
    /// Curve read-write access.
    crv_curve_access_callback: extern "C" fn(*const c_void, u16) -> Model709ReadOnly,
    /// Number Of Active Points (ActPt)
    ///
    /// Number of active points in must trip curve.
...
impl ReadAdapter for Model709CallbackAdapter {
    fn der_trip_lf_module_enable(&self) -> Ena {
...
impl WriteAdapter for Model709CallbackAdapter {
    /// DER Trip LF Module Enable (Ena)
    ///
    /// DER low frequency trip control enable.
    fn set_der_trip_lf_module_enable(&mut self, value: Ena) {
...
#[repr(C)]
pub struct Model709StatefulAdapter<const STORED_CURVE_COUNT: usize, const NUMBER_OF_POINTS: usize> {
    /// DER Trip LF Module Enable (Ena)
    ///
    /// DER low frequency trip control enable.
    pub der_trip_lf_module_enable: Model709Ena,
    /// Adopt Curve Request (AdptCrvReq)
    ///
    /// Index of curve points to adopt. First curve index is 1.
    ...
    ///
    /// Result of last adopt curve operation.
    pub adopt_curve_result: Model709AdptCrvRslt,
    /// Number Of Points (NPt)
    ///
    /// Number of curve points supported.
    pub number_of_points: u16,
    ...
    pub stored_curves: [Model709StoredCurves<NUMBER_OF_POINTS>; STORED_CURVE_COUNT],
}

#[repr(C)]
pub struct Model709StoredCurves<const NUMBER_OF_POINTS: usize> {
    /// Curve Access (ReadOnly)
    ///
    /// Curve read-write access.
    pub crv_curve_access: Model709ReadOnly,
    /// Number Of Active Points (ActPt)
    ///
    /// Number of active points in must trip curve.
    ...
    pub must_trip_curve_must_trip_curve_points:
        [Model709MustTripCurveMustTripCurvePoints; NUMBER_OF_POINTS],
    pub may_trip_curve_may_trip_curve_points:
        [Model709MayTripCurveMayTripCurvePoints; NUMBER_OF_POINTS],
    pub momentary_cessation_curve_mom_cessation_curve_points:
        [Model709MomentaryCessationCurveMomCessationCurvePoints; NUMBER_OF_POINTS],
}

#[repr(C)]
pub struct Model709MustTripCurveMustTripCurvePoints {
    /// Frequency Point (Hz)
    ///
    /// Curve frequency point.
    ///
...
#[repr(C)]
pub struct Model709MayTripCurveMayTripCurvePoints {
    /// Frequency Point (Hz)
    ///
    /// Curve frequency point.
...
#[repr(C)]
pub struct Model709MomentaryCessationCurveMomCessationCurvePoints {
    /// Frequency Point (Hz)
    ///
    /// Curve frequency point.
...
impl<const STORED_CURVE_COUNT: usize, const NUMBER_OF_POINTS: usize> ReadAdapter
    for Model709StatefulAdapter<STORED_CURVE_COUNT, NUMBER_OF_POINTS>
{
    fn der_trip_lf_module_enable(&self) -> Ena {
...
/// C-FFI dispatch descriptor for SunSpec model 709. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_709: StaticModelSpec = StaticModelSpec {
    id: 709,
    length: model_709_c_length,
    writable: true,
    visit_read: model_709_c_visit_read,
    visit_write: model_709_c_visit_write,
};

fn model_709_c_length(repeat_count_0: u16, repeat_count_1: u16) -> u16 {
    (Model709 {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model709{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_709_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        2 => Some(unsafe { &*(adapter as *const Model709CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_709_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_709_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    repeat_count_0: u16,
    repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model709CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
