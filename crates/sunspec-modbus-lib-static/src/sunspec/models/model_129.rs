...
use sunspec_modbus_lib_rs::sunspec::models::model_129::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model129CallbackAdapter`, valid for the
...
unsafe fn model_129_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model129CallbackAdapter).as_ref() };
...
unsafe fn model_129_c_visit_write(
    ...
    let model = Model129;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model129CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_129_callback(
    adapter: *mut Model129CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_129,
