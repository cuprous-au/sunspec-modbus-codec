...
use sunspec_modbus_lib_rs::sunspec::models::model_143::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model143CallbackAdapter`, valid for the
...
unsafe fn model_143_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model143CallbackAdapter).as_ref() };
...
unsafe fn model_143_c_visit_write(
    ...
    let model = Model143;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model143CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_143_callback(
    adapter: *mut Model143CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_143,
