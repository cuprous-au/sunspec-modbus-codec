...
/// Marker for SunSpec model 111. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model111;

impl<'ad> ModelSpec<'ad> for Model111 {
    const MODEL_ID: u16 = 111;
...
impl ReadAdapter for Model111StatefulAdapter {
    fn amps(&self) -> f32 {
        self.amps
    }
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model111{Stateful,Callback}Adapter`,
...
unsafe fn model_111_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model111StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model111CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
