...
use sunspec_modbus_lib_rs::sunspec::models::model_136::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model136CallbackAdapter`, valid for the
...
unsafe fn model_136_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model136CallbackAdapter).as_ref() };
...
unsafe fn model_136_c_visit_write(
    ...
    let model = Model136;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model136CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_136_callback(
    adapter: *mut Model136CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_136,
