...
/// Marker for SunSpec model 135. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model135;

impl<'ad> ModelSpec<'ad> for Model135 {
    const MODEL_ID: u16 = 135;
...
#[repr(u16)]
pub enum Model135ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model135ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model135ReadOnly> {
        match repr {
            0 => Some(Model135ReadOnly::Readwrite),
            1 => Some(Model135ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model135ReadOnly`] - only `Model135ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model135ReadOnly;
...
pub struct Model135StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model135ReadOnly,
}

impl ReadAdapter for Model135StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model135StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model135{Stateful,Callback}Adapter`,
...
unsafe fn model_135_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model135StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model135CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_135_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model135StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model135CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
