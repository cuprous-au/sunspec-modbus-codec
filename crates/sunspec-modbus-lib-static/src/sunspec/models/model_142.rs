...
use sunspec_modbus_lib_rs::sunspec::models::model_142::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model142CallbackAdapter`, valid for the
...
unsafe fn model_142_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model142CallbackAdapter).as_ref() };
...
unsafe fn model_142_c_visit_write(
    ...
    let model = Model142;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model142CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_142_callback(
    adapter: *mut Model142CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_142,
