...
use sunspec_modbus_lib_rs::sunspec::models::model_706::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model706CallbackAdapter`, valid for the
...
unsafe fn model_706_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model706CallbackAdapter).as_ref() };
...
unsafe fn model_706_c_visit_write(
    ...
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model706CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_706_callback(
    adapter: *mut Model706CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_706,
