...
/// Marker for SunSpec model 140. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model140;

impl<'ad> ModelSpec<'ad> for Model140 {
    const MODEL_ID: u16 = 140;
...
#[repr(u16)]
pub enum Model140CrvType {
    CeaseToEnergize = 1,
}

impl Model140CrvType {
    pub fn from_repr(repr: u16) -> Option<Model140CrvType> {
        match repr {
            1 => Some(Model140CrvType::CeaseToEnergize),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model140CrvType`] - only `Model140CrvType` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `CrvType` distinct from another model's point of the same name.
pub type CrvType = Model140CrvType;

#[derive(Clone, Copy, Debug, PartialEq)]
#[repr(u16)]
pub enum Model140ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model140ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model140ReadOnly> {
        match repr {
            0 => Some(Model140ReadOnly::Readwrite),
            1 => Some(Model140ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model140ReadOnly`] - only `Model140ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model140ReadOnly;
...
pub struct Model140CallbackAdapter {
    ...
    /// CrvType
    crv_type_callback: extern "C" fn(*const c_void) -> Model140CrvType,
    /// ActPt (ActPt)
    ///
    /// Number of active points in array.
    ...
    ///
    /// Curve is read-only or can be modified.
    curve_read_only_callback: extern "C" fn(*const c_void) -> Model140ReadOnly,
}

impl ReadAdapter for Model140CallbackAdapter {
    fn act_crv(&self) -> u16 {
...
impl WriteAdapter for Model140CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
pub struct Model140StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model140ReadOnly,
}

impl ReadAdapter for Model140StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model140StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 140. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_140: StaticModelSpec = StaticModelSpec {
    id: 140,
    length: model_140_c_length,
    writable: true,
    visit_read: model_140_c_visit_read,
    visit_write: model_140_c_visit_write,
};

fn model_140_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model140).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model140{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_140_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model140StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model140CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_140_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_140_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model140StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model140CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
