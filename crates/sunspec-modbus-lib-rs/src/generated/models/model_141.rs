...
/// Marker for SunSpec model 141. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model141;

impl<'ad> ModelSpec<'ad> for Model141 {
    const MODEL_ID: u16 = 141;
...
#[repr(u16)]
pub enum Model141ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model141ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model141ReadOnly> {
        match repr {
            0 => Some(Model141ReadOnly::Readwrite),
            1 => Some(Model141ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model141ReadOnly`] - only `Model141ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model141ReadOnly;
...
pub struct Model141CallbackAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    curve_read_only_callback: extern "C" fn(*const c_void) -> Model141ReadOnly,
}

impl ReadAdapter for Model141CallbackAdapter {
    fn act_crv(&self) -> u16 {
...
impl WriteAdapter for Model141CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
pub struct Model141StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model141ReadOnly,
}

impl ReadAdapter for Model141StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model141StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 141. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_141: StaticModelSpec = StaticModelSpec {
    id: 141,
    length: model_141_c_length,
    writable: true,
    visit_read: model_141_c_visit_read,
    visit_write: model_141_c_visit_write,
};

fn model_141_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model141).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model141{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_141_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model141StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model141CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_141_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_141_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model141StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model141CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
