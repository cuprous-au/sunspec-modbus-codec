...
use sunspec_modbus_lib_rs::sunspec::models::model_144::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model144CallbackAdapter`, valid for the
...
unsafe fn model_144_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model144CallbackAdapter).as_ref() };
...
unsafe fn model_144_c_visit_write(
    ...
    let model = Model144;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model144CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_144_callback(
    adapter: *mut Model144CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_144,
