#![no_std]
use core::{
    ffi::{CStr, c_char, c_uint}, panic::PanicInfo, slice,
};

use sunspec_modbus_lib_rs::{ModbusRequest, handle_request, sunspec::adapters::SunspecExternalAdapters};
...
pub extern "C" fn sunspec_service_handle_request(
    ...
    let res = match handle_request(adapter_ref, ModbusRequest::ReadRegister(address, length), buf) {
        Ok(()) => 0,
        Err(_) => -2,
    };
