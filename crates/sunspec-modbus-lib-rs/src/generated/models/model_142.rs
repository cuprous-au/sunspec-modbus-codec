...
pub struct Model142CallbackAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    curve_read_only_callback: extern "C" fn(*const c_void) -> Model142ReadOnly,
}

impl ReadAdapter for Model142CallbackAdapter {
    fn act_crv(&self) -> u16 {
...
impl WriteAdapter for Model142CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
pub struct Model142StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model142ReadOnly,
}

impl ReadAdapter for Model142StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model142StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 142. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_142: StaticModelSpec = StaticModelSpec {
    id: 142,
    length: model_142_c_length,
    writable: true,
    visit_read: model_142_c_visit_read,
    visit_write: model_142_c_visit_write,
};

fn model_142_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model142).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model142{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_142_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model142StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model142CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_142_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_142_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model142StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model142CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
