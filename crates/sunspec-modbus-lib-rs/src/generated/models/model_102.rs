...
/// Marker for SunSpec model 102. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model102;

impl<'ad> ModelSpec<'ad> for Model102 {
    const MODEL_ID: u16 = 102;
...
impl ReadAdapter for Model102CallbackAdapter {
    fn amps(&self) -> u16 {
...
impl ReadAdapter for Model102StatefulAdapter {
    fn amps(&self) -> u16 {
        self.amps
    }
...
/// C-FFI dispatch descriptor for SunSpec model 102. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_102: StaticModelSpec = StaticModelSpec {
    id: 102,
    length: model_102_c_length,
    writable: false,
    visit_read: model_102_c_visit_read,
    visit_write: model_102_c_visit_write,
};

fn model_102_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model102).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model102{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_102_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model102StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model102CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_102_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_102_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
