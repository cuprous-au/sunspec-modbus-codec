...
/// Marker for SunSpec model 113. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model113;

impl<'ad> ModelSpec<'ad> for Model113 {
    const MODEL_ID: u16 = 113;
...
impl Model113St {
    pub fn from_repr(repr: u16) -> Option<Model113St> {
        match repr {
            1 => Some(Model113St::Off),
            2 => Some(Model113St::Sleeping),
            3 => Some(Model113St::Starting),
            4 => Some(Model113St::Mppt),
            5 => Some(Model113St::Throttled),
            6 => Some(Model113St::ShuttingDown),
            7 => Some(Model113St::Fault),
            8 => Some(Model113St::Standby),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model113St`] - only `Model113St` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `St` distinct from another model's point of the same name.
pub type St = Model113St;
...
pub struct Model113CallbackAdapter {
    ...
    ///
    /// Operating state
    operating_state_callback: extern "C" fn(*const c_void) -> Model113St,
    /// Vendor Operating State (StVnd)
    ///
    /// Vendor specific operating state code
...
impl ReadAdapter for Model113CallbackAdapter {
    fn amps(&self) -> f32 {
...
pub struct Model113StatefulAdapter {
    ...
    ///
    /// Operating state
    pub operating_state: Model113St,
    /// Vendor Operating State (StVnd)
    ///
    /// Vendor specific operating state code
...
impl ReadAdapter for Model113StatefulAdapter {
    fn amps(&self) -> f32 {
        self.amps
    }
...
/// C-FFI dispatch descriptor for SunSpec model 113. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_113: StaticModelSpec = StaticModelSpec {
    id: 113,
    length: model_113_c_length,
    writable: false,
    visit_read: model_113_c_visit_read,
    visit_write: model_113_c_visit_write,
};

fn model_113_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model113).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model113{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_113_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model113StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model113CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_113_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_113_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
