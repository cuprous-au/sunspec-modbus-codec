...
impl WriteAdapter for Model131StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model131{Stateful,Callback}Adapter`,
...
unsafe fn model_131_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model131StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model131CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_131_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model131StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model131CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
