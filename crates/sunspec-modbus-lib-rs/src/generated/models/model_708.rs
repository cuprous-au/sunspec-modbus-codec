...
/// Marker for SunSpec model 708. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model708 {
...
impl<'ad> ModelSpec<'ad> for Model708 {
    const MODEL_ID: u16 = 708;
...
pub(crate) fn read_point_from_buffer<'a>(
    ...
    match point {
        Point::DerTripHvModuleEnable => {
            ...
            adapter.set_der_trip_hv_module_enable(enum_value);
            Ok(())
        }
        Point::AdoptCurveRequest => {
            let value = buffer.read_u16()?;
...
#[repr(C)]
pub struct Model708StatefulAdapter<const STORED_CURVE_COUNT: usize, const NUMBER_OF_POINTS: usize> {
    /// DER Trip HV Module Enable (Ena)
    ///
    /// DER high voltage trip control enable.
    ...
    pub stored_curves: [Model708StoredCurves<NUMBER_OF_POINTS>; STORED_CURVE_COUNT],
}

#[repr(C)]
pub struct Model708StoredCurves<const NUMBER_OF_POINTS: usize> {
    /// Curve Access (ReadOnly)
    ///
    /// Curve read-write access.
    ...
    pub must_trip_curve_must_trip_curve_points:
        [Model708MustTripCurveMustTripCurvePoints; NUMBER_OF_POINTS],
    pub may_trip_curve_may_trip_curve_points:
        [Model708MayTripCurveMayTripCurvePoints; NUMBER_OF_POINTS],
    pub momentary_cessation_curve_mom_cessation_curve_points:
        [Model708MomentaryCessationCurveMomCessationCurvePoints; NUMBER_OF_POINTS],
}

#[repr(C)]
pub struct Model708MustTripCurveMustTripCurvePoints {
    /// Voltage Point (V)
    ///
    /// Curve voltage point as percentage.
    ///
...
#[repr(C)]
pub struct Model708MayTripCurveMayTripCurvePoints {
    /// Voltage Point (V)
    ///
    /// Curve voltage point as percentage.
...
#[repr(C)]
pub struct Model708MomentaryCessationCurveMomCessationCurvePoints {
    /// Voltage Point (V)
    ///
    /// Curve voltage point as percentage.
...
impl<const STORED_CURVE_COUNT: usize, const NUMBER_OF_POINTS: usize> ReadAdapter
    for Model708StatefulAdapter<STORED_CURVE_COUNT, NUMBER_OF_POINTS>
{
    fn der_trip_hv_module_enable(&self) -> Ena {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model708{Stateful,Callback}Adapter`,
...
unsafe fn model_708_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        2 => Some(unsafe { &*(adapter as *const Model708CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_708_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model708CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
