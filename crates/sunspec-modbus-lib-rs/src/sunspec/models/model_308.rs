...
impl ReadAdapter for Model308StatefulAdapter {
    fn ghi(&self) -> Option<u16> {
        Some(self.ghi)
    }
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model308{Stateful,Callback}Adapter`,
...
unsafe fn model_308_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model308StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model308CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
