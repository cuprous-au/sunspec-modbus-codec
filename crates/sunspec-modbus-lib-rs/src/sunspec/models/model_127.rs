...
impl ReadAdapter for Model127StatefulAdapter {
    fn w_gra(&self) -> u16 {
        self.w_gra
    }
...
impl WriteAdapter for Model127StatefulAdapter {
    /// WGra (WGra)
    ///
    /// The slope of the reduction in the maximum allowed watts output as a function of frequency.
    fn set_w_gra(&mut self, value: u16) {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model127{Stateful,Callback}Adapter`,
...
unsafe fn model_127_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model127StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model127CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_127_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model127StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model127CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
