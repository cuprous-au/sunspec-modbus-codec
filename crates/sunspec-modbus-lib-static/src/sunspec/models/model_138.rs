...
use sunspec_modbus_lib_rs::sunspec::models::model_138::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model138CallbackAdapter`, valid for the
...
unsafe fn model_138_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model138CallbackAdapter).as_ref() };
...
unsafe fn model_138_c_visit_write(
    ...
    let model = Model138;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model138CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_138_callback(
    adapter: *mut Model138CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_138,
