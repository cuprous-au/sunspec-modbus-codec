...
pub fn write_point<'a>(model: &dyn ModelAdapter, point: &Point, buffer: ModbusBuffer<'a>, offset: u16, limit: u16) {
    match point {
        ...
        Point::AmpsPhaseB => {
            buffer::write_u16(model.amps_phase_b(), buffer);
        },
        Point::AmpsPhaseC => {
            buffer::write_u16(model.amps_phase_c(), buffer);
        },
        Point::ASf => {
        ...
        Point::PhaseVoltageBn => {
            buffer::write_u16(model.phase_voltage_bn(), buffer);
        },
        Point::PhaseVoltageCn => {
            buffer::write_u16(model.phase_voltage_cn(), buffer);
        },
        Point::VSf => {
