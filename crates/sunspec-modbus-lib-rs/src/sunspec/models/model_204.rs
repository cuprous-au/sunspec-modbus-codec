...
/// Marker for SunSpec model 204. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model204;

impl<'ad> ModelSpec<'ad> for Model204 {
    const MODEL_ID: u16 = 204;
...
impl ReadAdapter for Model204CallbackAdapter {
    fn amps(&self) -> i16 {
...
impl ReadAdapter for Model204StatefulAdapter {
    fn amps(&self) -> i16 {
        self.amps
    }
...
/// C-FFI dispatch descriptor for SunSpec model 204. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_204: StaticModelSpec = StaticModelSpec {
    id: 204,
    length: model_204_c_length,
    writable: false,
    visit_read: model_204_c_visit_read,
    visit_write: model_204_c_visit_write,
};

fn model_204_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model204).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model204{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_204_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model204StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model204CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_204_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_204_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
