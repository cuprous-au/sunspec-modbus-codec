...
/// Marker for SunSpec model 137. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model137;

impl<'ad> ModelSpec<'ad> for Model137 {
    const MODEL_ID: u16 = 137;
...
#[repr(u16)]
pub enum Model137ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model137ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model137ReadOnly> {
        match repr {
            0 => Some(Model137ReadOnly::Readwrite),
            1 => Some(Model137ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model137ReadOnly`] - only `Model137ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model137ReadOnly;
...
pub struct Model137StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model137ReadOnly,
}

impl ReadAdapter for Model137StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model137StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model137{Stateful,Callback}Adapter`,
...
unsafe fn model_137_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model137StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model137CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
unsafe fn model_137_c_visit_write(
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model137StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model137CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
