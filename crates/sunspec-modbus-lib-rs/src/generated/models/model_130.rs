...
/// Marker for SunSpec model 130. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model130;

impl<'ad> ModelSpec<'ad> for Model130 {
    const MODEL_ID: u16 = 130;
...
#[repr(u16)]
pub enum Model130ReadOnly {
    Readwrite = 0,
    Readonly = 1,
}

impl Model130ReadOnly {
    pub fn from_repr(repr: u16) -> Option<Model130ReadOnly> {
        match repr {
            0 => Some(Model130ReadOnly::Readwrite),
            1 => Some(Model130ReadOnly::Readonly),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model130ReadOnly`] - only `Model130ReadOnly` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `ReadOnly` distinct from another model's point of the same name.
pub type ReadOnly = Model130ReadOnly;
...
pub struct Model130CallbackAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    curve_read_only_callback: extern "C" fn(*const c_void) -> Model130ReadOnly,
}

impl ReadAdapter for Model130CallbackAdapter {
    fn act_crv(&self) -> u16 {
...
impl WriteAdapter for Model130CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
pub struct Model130StatefulAdapter {
    ...
    ///
    /// Curve is read-only or can be modified.
    pub curve_read_only: Model130ReadOnly,
}

impl ReadAdapter for Model130StatefulAdapter {
    fn act_crv(&self) -> u16 {
        self.act_crv
    }
...
impl WriteAdapter for Model130StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 130. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_130: StaticModelSpec = StaticModelSpec {
    id: 130,
    length: model_130_c_length,
    writable: true,
    visit_read: model_130_c_visit_read,
    visit_write: model_130_c_visit_write,
};

fn model_130_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model130).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model130{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_130_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model130StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model130CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_130_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_130_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model130StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model130CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
