...
impl ReadAdapter for Model403CallbackAdapter {
    fn dca_sf(&self) -> i16 {
...
impl ReadAdapter for Model403StatefulAdapter {
    fn dca_sf(&self) -> i16 {
        self.dca_sf
    }
...
/// C-FFI dispatch descriptor for SunSpec model 403. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_403: StaticModelSpec = StaticModelSpec {
    id: 403,
    length: model_403_c_length,
    writable: false,
    visit_read: model_403_c_visit_read,
    visit_write: model_403_c_visit_write,
};

fn model_403_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model403).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model403{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_403_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model403StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model403CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_403_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_403_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
