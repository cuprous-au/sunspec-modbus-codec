...
/// Marker for SunSpec model 203. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model203;

impl<'ad> ModelSpec<'ad> for Model203 {
    const MODEL_ID: u16 = 203;
...
pub(crate) fn write_point_to_buffer<'a>(
    ...
    match point {
        ...
        Point::PhaseVoltageCn => {
            buffer.write_i16(adapter.phase_voltage_cn());
        }
        Point::VoltageLl => {
        ...
        Point::PhaseVoltageBc => {
            buffer.write_i16(adapter.phase_voltage_bc());
        }
        Point::PhaseVoltageCa => {
            buffer.write_i16(adapter.phase_voltage_ca());
        }
        Point::VSf => {
...
impl ReadAdapter for Model203CallbackAdapter {
    fn amps(&self) -> i16 {
...
impl ReadAdapter for Model203StatefulAdapter {
    fn amps(&self) -> i16 {
        self.amps
    }
...
/// C-FFI dispatch descriptor for SunSpec model 203. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_203: StaticModelSpec = StaticModelSpec {
    id: 203,
    length: model_203_c_length,
    writable: false,
    visit_read: model_203_c_visit_read,
    visit_write: model_203_c_visit_write,
};

fn model_203_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model203).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model203{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_203_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model203StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model203CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_203_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_203_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
