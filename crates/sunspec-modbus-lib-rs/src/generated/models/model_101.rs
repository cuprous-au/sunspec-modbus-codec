// @generated by the sunspec-gen crate (crates/sunspec-gen) from the SunSpec model
// definitions in https://github.com/sunspec/models. Do not edit by hand - run
// `cargo run -p sunspec-gen` to regenerate.

use crate::ModbusException;
use crate::buffer::{ReadableRegisterBuffer, WritableRegisterBuffer};
use crate::cursor::Cursor;
use crate::model::{ModelSpec, StaticModelSpec};
use core::cmp::min;
use core::ffi::c_void;

static POINTS: [PointDetails<()>; 45] = [
    PointDetails {
        point: |()| Point::ModelId,
        size: 1,
        start_address: 0,
    },
    PointDetails {
        point: |()| Point::ModelLength,
        size: 1,
        start_address: 1,
    },
    PointDetails {
        point: |()| Point::Amps,
        size: 1,
        start_address: 2,
    },
    PointDetails {
        point: |()| Point::AmpsPhaseA,
        size: 1,
        start_address: 3,
    },
    PointDetails {
        point: |()| Point::AmpsPhaseB,
        size: 1,
        start_address: 4,
    },
    PointDetails {
        point: |()| Point::AmpsPhaseC,
        size: 1,
        start_address: 5,
    },
    PointDetails {
        point: |()| Point::ASf,
        size: 1,
        start_address: 6,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageAb,
        size: 1,
        start_address: 7,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageBc,
        size: 1,
        start_address: 8,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageCa,
        size: 1,
        start_address: 9,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageAn,
        size: 1,
        start_address: 10,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageBn,
        size: 1,
        start_address: 11,
    },
    PointDetails {
        point: |()| Point::PhaseVoltageCn,
        size: 1,
        start_address: 12,
    },
    PointDetails {
        point: |()| Point::VSf,
        size: 1,
        start_address: 13,
    },
    PointDetails {
        point: |()| Point::Watts,
        size: 1,
        start_address: 14,
    },
    PointDetails {
        point: |()| Point::WSf,
        size: 1,
        start_address: 15,
    },
    PointDetails {
        point: |()| Point::Hz,
        size: 1,
        start_address: 16,
    },
    PointDetails {
        point: |()| Point::HzSf,
        size: 1,
        start_address: 17,
    },
    PointDetails {
        point: |()| Point::Va,
        size: 1,
        start_address: 18,
    },
    PointDetails {
        point: |()| Point::VaSf,
        size: 1,
        start_address: 19,
    },
    PointDetails {
        point: |()| Point::VAr,
        size: 1,
        start_address: 20,
    },
    PointDetails {
        point: |()| Point::VArSf,
        size: 1,
        start_address: 21,
    },
    PointDetails {
        point: |()| Point::Pf,
        size: 1,
        start_address: 22,
    },
    PointDetails {
        point: |()| Point::PfSf,
        size: 1,
        start_address: 23,
    },
    PointDetails {
        point: |()| Point::WattHours,
        size: 2,
        start_address: 24,
    },
    PointDetails {
        point: |()| Point::WhSf,
        size: 1,
        start_address: 26,
    },
    PointDetails {
        point: |()| Point::DcAmps,
        size: 1,
        start_address: 27,
    },
    PointDetails {
        point: |()| Point::DcaSf,
        size: 1,
        start_address: 28,
    },
    PointDetails {
        point: |()| Point::DcVoltage,
        size: 1,
        start_address: 29,
    },
    PointDetails {
        point: |()| Point::DcvSf,
        size: 1,
        start_address: 30,
    },
    PointDetails {
        point: |()| Point::DcWatts,
        size: 1,
        start_address: 31,
    },
    PointDetails {
        point: |()| Point::DcwSf,
        size: 1,
        start_address: 32,
    },
    PointDetails {
        point: |()| Point::CabinetTemperature,
        size: 1,
        start_address: 33,
    },
    PointDetails {
        point: |()| Point::HeatSinkTemperature,
        size: 1,
        start_address: 34,
    },
    PointDetails {
        point: |()| Point::TransformerTemperature,
        size: 1,
        start_address: 35,
    },
    PointDetails {
        point: |()| Point::OtherTemperature,
        size: 1,
        start_address: 36,
    },
    PointDetails {
        point: |()| Point::TmpSf,
        size: 1,
        start_address: 37,
    },
    PointDetails {
        point: |()| Point::OperatingState,
        size: 1,
        start_address: 38,
    },
    PointDetails {
        point: |()| Point::VendorOperatingState,
        size: 1,
        start_address: 39,
    },
    PointDetails {
        point: |()| Point::Event1,
        size: 2,
        start_address: 40,
    },
    PointDetails {
        point: |()| Point::EventBitfield2,
        size: 2,
        start_address: 42,
    },
    PointDetails {
        point: |()| Point::VendorEventBitfield1,
        size: 2,
        start_address: 44,
    },
    PointDetails {
        point: |()| Point::VendorEventBitfield2,
        size: 2,
        start_address: 46,
    },
    PointDetails {
        point: |()| Point::VendorEventBitfield3,
        size: 2,
        start_address: 48,
    },
    PointDetails {
        point: |()| Point::VendorEventBitfield4,
        size: 2,
        start_address: 50,
    },
];

#[derive(Debug)]
pub enum Point {
    ModelId,
    ModelLength,
    Amps,
    AmpsPhaseA,
    AmpsPhaseB,
    AmpsPhaseC,
    ASf,
    PhaseVoltageAb,
    PhaseVoltageBc,
    PhaseVoltageCa,
    PhaseVoltageAn,
    PhaseVoltageBn,
    PhaseVoltageCn,
    VSf,
    Watts,
    WSf,
    Hz,
    HzSf,
    Va,
    VaSf,
    VAr,
    VArSf,
    Pf,
    PfSf,
    WattHours,
    WhSf,
    DcAmps,
    DcaSf,
    DcVoltage,
    DcvSf,
    DcWatts,
    DcwSf,
    CabinetTemperature,
    HeatSinkTemperature,
    TransformerTemperature,
    OtherTemperature,
    TmpSf,
    OperatingState,
    VendorOperatingState,
    Event1,
    EventBitfield2,
    VendorEventBitfield1,
    VendorEventBitfield2,
    VendorEventBitfield3,
    VendorEventBitfield4,
}

struct PointDetails<GroupIndexArgs> {
    point: fn(GroupIndexArgs) -> Point,
    start_address: u16,
    size: u16,
}

/// Marker for SunSpec model 101. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model101;

impl<'ad> ModelSpec<'ad> for Model101 {
    const MODEL_ID: u16 = 101;
    type ReadAdapter = dyn ReadAdapter + 'ad;
    type WriteAdapter = dyn WriteAdapter + 'ad;

    fn model_length(&self) -> u16 {
        52
    }

    fn traverse_points_read<'a>(
        &self,
        adapter: &Self::ReadAdapter,
        buffer: &mut WritableRegisterBuffer<'a>,
        offset: u16,
    ) -> Result<(), ModbusException> {
        let until = offset + buffer.len();
        let mut cursor = 0;

        let iter = POINTS
            .iter()
            .map(|p| (p.start_address, p.size, (p.point)(())))
            .skip_while(|(start, size, _)| offset >= start + size)
            .take_while(|(start, _, _)| until > *start);

        for (start, size, point) in iter {
            let point_offset = offset.saturating_sub(start);
            let word_count = min(size - point_offset, buffer.len() - cursor);
            write_point_to_buffer(
                self,
                adapter,
                &point,
                &mut buffer.slice(cursor, word_count),
                point_offset,
            );
            cursor += word_count;
        }

        Ok(())
    }

    fn traverse_points_write<'a>(
        &self,
        adapter: &mut Self::WriteAdapter,
        buffer: &ReadableRegisterBuffer<'a>,
        offset: u16,
    ) -> Result<(), ModbusException> {
        let until = offset + buffer.len();
        let mut cursor = 0;

        let iter = POINTS
            .iter()
            .map(|p| (p.start_address, p.size, (p.point)(())))
            .skip_while(|(start, size, _)| offset >= start + size)
            .take_while(|(start, _, _)| until > *start);

        for (start, size, point) in iter {
            let point_offset = offset.saturating_sub(start);
            let word_count = min(size - point_offset, buffer.len() - cursor);
            if point_offset > 0 {
                return Err(ModbusException::IllegalDataAddress);
            }
            read_point_from_buffer(self, adapter, &point, &buffer.slice(cursor, word_count))?;
            cursor += word_count;
        }

        Ok(())
    }
}

pub(crate) fn write_point_to_buffer<'a>(
    model: &Model101,
    adapter: &dyn ReadAdapter,
    point: &Point,
    buffer: &mut WritableRegisterBuffer<'a>,
    offset: u16,
) {
    match point {
        Point::ModelId => {
            buffer.write_u16(101);
        }
        Point::ModelLength => {
            buffer.write_u16(model.model_length() - 2);
        }
        Point::Amps => {
            buffer.write_u16(adapter.amps());
        }
        Point::AmpsPhaseA => {
            buffer.write_u16(adapter.amps_phase_a());
        }
        Point::AmpsPhaseB => {
            if let Some(value) = adapter.amps_phase_b() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::AmpsPhaseC => {
            if let Some(value) = adapter.amps_phase_c() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::ASf => {
            buffer.write_i16(adapter.a_sf());
        }
        Point::PhaseVoltageAb => {
            if let Some(value) = adapter.phase_voltage_ab() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::PhaseVoltageBc => {
            if let Some(value) = adapter.phase_voltage_bc() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::PhaseVoltageCa => {
            if let Some(value) = adapter.phase_voltage_ca() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::PhaseVoltageAn => {
            buffer.write_u16(adapter.phase_voltage_an());
        }
        Point::PhaseVoltageBn => {
            if let Some(value) = adapter.phase_voltage_bn() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::PhaseVoltageCn => {
            if let Some(value) = adapter.phase_voltage_cn() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::VSf => {
            buffer.write_i16(adapter.v_sf());
        }
        Point::Watts => {
            buffer.write_i16(adapter.watts());
        }
        Point::WSf => {
            buffer.write_i16(adapter.w_sf());
        }
        Point::Hz => {
            buffer.write_u16(adapter.hz());
        }
        Point::HzSf => {
            buffer.write_i16(adapter.hz_sf());
        }
        Point::Va => {
            if let Some(value) = adapter.va() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::VaSf => {
            if let Some(value) = adapter.va_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::VAr => {
            if let Some(value) = adapter.v_ar() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::VArSf => {
            if let Some(value) = adapter.v_ar_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::Pf => {
            if let Some(value) = adapter.pf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::PfSf => {
            if let Some(value) = adapter.pf_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::WattHours => {
            buffer.write_u32(adapter.watt_hours(), offset);
        }
        Point::WhSf => {
            buffer.write_i16(adapter.wh_sf());
        }
        Point::DcAmps => {
            if let Some(value) = adapter.dc_amps() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::DcaSf => {
            if let Some(value) = adapter.dca_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::DcVoltage => {
            if let Some(value) = adapter.dc_voltage() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::DcvSf => {
            if let Some(value) = adapter.dcv_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::DcWatts => {
            if let Some(value) = adapter.dc_watts() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::DcwSf => {
            if let Some(value) = adapter.dcw_sf() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::CabinetTemperature => {
            buffer.write_i16(adapter.cabinet_temperature());
        }
        Point::HeatSinkTemperature => {
            if let Some(value) = adapter.heat_sink_temperature() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::TransformerTemperature => {
            if let Some(value) = adapter.transformer_temperature() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::OtherTemperature => {
            if let Some(value) = adapter.other_temperature() {
                buffer.write_i16(value);
            } else {
                buffer.zero();
            }
        }
        Point::TmpSf => {
            buffer.write_i16(adapter.tmp_sf());
        }
        Point::OperatingState => {
            buffer.write_u16(adapter.operating_state() as u16);
        }
        Point::VendorOperatingState => {
            if let Some(value) = adapter.vendor_operating_state() {
                buffer.write_u16(value);
            } else {
                buffer.zero();
            }
        }
        Point::Event1 => {
            buffer.write_u32(adapter.event1(), offset);
        }
        Point::EventBitfield2 => {
            buffer.write_u32(adapter.event_bitfield_2(), offset);
        }
        Point::VendorEventBitfield1 => {
            if let Some(value) = adapter.vendor_event_bitfield_1() {
                buffer.write_u32(value, offset);
            } else {
                buffer.zero();
            }
        }
        Point::VendorEventBitfield2 => {
            if let Some(value) = adapter.vendor_event_bitfield_2() {
                buffer.write_u32(value, offset);
            } else {
                buffer.zero();
            }
        }
        Point::VendorEventBitfield3 => {
            if let Some(value) = adapter.vendor_event_bitfield_3() {
                buffer.write_u32(value, offset);
            } else {
                buffer.zero();
            }
        }
        Point::VendorEventBitfield4 => {
            if let Some(value) = adapter.vendor_event_bitfield_4() {
                buffer.write_u32(value, offset);
            } else {
                buffer.zero();
            }
        }
    }
}

pub(crate) fn read_point_from_buffer<'a>(
    model: &Model101,
    adapter: &mut dyn WriteAdapter,
    point: &Point,
    buffer: &ReadableRegisterBuffer<'a>,
) -> Result<(), ModbusException> {
    Err(ModbusException::IllegalDataAddress)
}

pub trait ReadAdapter {
    /// Amps (A)
    ///
    /// AC Current
    fn amps(&self) -> u16;

    /// Amps PhaseA (AphA)
    ///
    /// Phase A Current
    ///
    /// Connected Phase
    fn amps_phase_a(&self) -> u16;

    /// Amps PhaseB (AphB)
    ///
    /// Phase B Current
    fn amps_phase_b(&self) -> Option<u16> {
        None
    }

    /// Amps PhaseC (AphC)
    ///
    /// Phase C Current
    fn amps_phase_c(&self) -> Option<u16> {
        None
    }

    /// A_SF
    fn a_sf(&self) -> i16;

    /// Phase Voltage AB (PPVphAB)
    ///
    /// Phase Voltage AB
    fn phase_voltage_ab(&self) -> Option<u16> {
        None
    }

    /// Phase Voltage BC (PPVphBC)
    ///
    /// Phase Voltage BC
    fn phase_voltage_bc(&self) -> Option<u16> {
        None
    }

    /// Phase Voltage CA (PPVphCA)
    ///
    /// Phase Voltage CA
    fn phase_voltage_ca(&self) -> Option<u16> {
        None
    }

    /// Phase Voltage AN (PhVphA)
    ///
    /// Phase Voltage AN
    fn phase_voltage_an(&self) -> u16;

    /// Phase Voltage BN (PhVphB)
    ///
    /// Phase Voltage BN
    fn phase_voltage_bn(&self) -> Option<u16> {
        None
    }

    /// Phase Voltage CN (PhVphC)
    ///
    /// Phase Voltage CN
    fn phase_voltage_cn(&self) -> Option<u16> {
        None
    }

    /// V_SF
    fn v_sf(&self) -> i16;

    /// Watts (W)
    ///
    /// AC Power
    fn watts(&self) -> i16;

    /// W_SF
    fn w_sf(&self) -> i16;

    /// Hz (Hz)
    ///
    /// Line Frequency
    fn hz(&self) -> u16;

    /// Hz_SF
    fn hz_sf(&self) -> i16;

    /// VA (VA)
    ///
    /// AC Apparent Power
    fn va(&self) -> Option<i16> {
        None
    }

    /// VA_SF
    fn va_sf(&self) -> Option<i16> {
        None
    }

    /// VAr (VAr)
    ///
    /// AC Reactive Power
    fn v_ar(&self) -> Option<i16> {
        None
    }

    /// VAr_SF
    fn v_ar_sf(&self) -> Option<i16> {
        None
    }

    /// PF (PF)
    ///
    /// AC Power Factor
    fn pf(&self) -> Option<i16> {
        None
    }

    /// PF_SF
    fn pf_sf(&self) -> Option<i16> {
        None
    }

    /// WattHours (WH)
    ///
    /// AC Energy
    fn watt_hours(&self) -> u32;

    /// WH_SF
    fn wh_sf(&self) -> i16;

    /// DC Amps (DCA)
    ///
    /// DC Current
    fn dc_amps(&self) -> Option<u16> {
        None
    }

    /// DCA_SF
    fn dca_sf(&self) -> Option<i16> {
        None
    }

    /// DC Voltage (DCV)
    ///
    /// DC Voltage
    fn dc_voltage(&self) -> Option<u16> {
        None
    }

    /// DCV_SF
    fn dcv_sf(&self) -> Option<i16> {
        None
    }

    /// DC Watts (DCW)
    ///
    /// DC Power
    fn dc_watts(&self) -> Option<i16> {
        None
    }

    /// DCW_SF
    fn dcw_sf(&self) -> Option<i16> {
        None
    }

    /// Cabinet Temperature (TmpCab)
    ///
    /// Cabinet Temperature
    fn cabinet_temperature(&self) -> i16;

    /// Heat Sink Temperature (TmpSnk)
    ///
    /// Heat Sink Temperature
    fn heat_sink_temperature(&self) -> Option<i16> {
        None
    }

    /// Transformer Temperature (TmpTrns)
    ///
    /// Transformer Temperature
    fn transformer_temperature(&self) -> Option<i16> {
        None
    }

    /// Other Temperature (TmpOt)
    ///
    /// Other Temperature
    fn other_temperature(&self) -> Option<i16> {
        None
    }

    /// Tmp_SF
    fn tmp_sf(&self) -> i16;

    /// Operating State (St)
    ///
    /// Operating state
    fn operating_state(&self) -> St;

    /// Vendor Operating State (StVnd)
    ///
    /// Vendor specific operating state code
    fn vendor_operating_state(&self) -> Option<u16> {
        None
    }

    /// Event1 (Evt1)
    ///
    /// Event fields
    fn event1(&self) -> u32;

    /// Event Bitfield 2 (Evt2)
    ///
    /// Reserved for future use
    fn event_bitfield_2(&self) -> u32;

    /// Vendor Event Bitfield 1 (EvtVnd1)
    ///
    /// Vendor defined events
    fn vendor_event_bitfield_1(&self) -> Option<u32> {
        None
    }

    /// Vendor Event Bitfield 2 (EvtVnd2)
    ///
    /// Vendor defined events
    fn vendor_event_bitfield_2(&self) -> Option<u32> {
        None
    }

    /// Vendor Event Bitfield 3 (EvtVnd3)
    ///
    /// Vendor defined events
    fn vendor_event_bitfield_3(&self) -> Option<u32> {
        None
    }

    /// Vendor Event Bitfield 4 (EvtVnd4)
    ///
    /// Vendor defined events
    fn vendor_event_bitfield_4(&self) -> Option<u32> {
        None
    }
}

pub trait WriteAdapter {}

#[derive(Clone, Copy, Debug, PartialEq)]
#[repr(u16)]
pub enum Model101St {
    Off = 1,
    Sleeping = 2,
    Starting = 3,
    Mppt = 4,
    Throttled = 5,
    ShuttingDown = 6,
    Fault = 7,
    Standby = 8,
}

impl Model101St {
    pub fn from_repr(repr: u16) -> Option<Model101St> {
        match repr {
            1 => Some(Model101St::Off),
            2 => Some(Model101St::Sleeping),
            3 => Some(Model101St::Starting),
            4 => Some(Model101St::Mppt),
            5 => Some(Model101St::Throttled),
            6 => Some(Model101St::ShuttingDown),
            7 => Some(Model101St::Fault),
            8 => Some(Model101St::Standby),
            _ => None,
        }
    }
}

/// Short, spec-matching alias for [`Model101St`] - only `Model101St` (unique per model) reaches the generated C header, since cbindgen has no module system to keep `St` distinct from another model's point of the same name.
pub type St = Model101St;

#[repr(C)]
pub struct Model101CallbackAdapter {
    context: *mut c_void,
    /// Amps (A)
    ///
    /// AC Current
    amps_callback: extern "C" fn(*const c_void) -> u16,
    /// Amps PhaseA (AphA)
    ///
    /// Phase A Current
    ///
    /// Connected Phase
    amps_phase_a_callback: extern "C" fn(*const c_void) -> u16,
    /// Amps PhaseB (AphB)
    ///
    /// Phase B Current
    amps_phase_b_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Amps PhaseC (AphC)
    ///
    /// Phase C Current
    amps_phase_c_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// A_SF
    a_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// Phase Voltage AB (PPVphAB)
    ///
    /// Phase Voltage AB
    phase_voltage_ab_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Phase Voltage BC (PPVphBC)
    ///
    /// Phase Voltage BC
    phase_voltage_bc_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Phase Voltage CA (PPVphCA)
    ///
    /// Phase Voltage CA
    phase_voltage_ca_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Phase Voltage AN (PhVphA)
    ///
    /// Phase Voltage AN
    phase_voltage_an_callback: extern "C" fn(*const c_void) -> u16,
    /// Phase Voltage BN (PhVphB)
    ///
    /// Phase Voltage BN
    phase_voltage_bn_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Phase Voltage CN (PhVphC)
    ///
    /// Phase Voltage CN
    phase_voltage_cn_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// V_SF
    v_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// Watts (W)
    ///
    /// AC Power
    watts_callback: extern "C" fn(*const c_void) -> i16,
    /// W_SF
    w_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// Hz (Hz)
    ///
    /// Line Frequency
    hz_callback: extern "C" fn(*const c_void) -> u16,
    /// Hz_SF
    hz_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// VA (VA)
    ///
    /// AC Apparent Power
    va_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// VA_SF
    va_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// VAr (VAr)
    ///
    /// AC Reactive Power
    v_ar_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// VAr_SF
    v_ar_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// PF (PF)
    ///
    /// AC Power Factor
    pf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// PF_SF
    pf_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// WattHours (WH)
    ///
    /// AC Energy
    watt_hours_callback: extern "C" fn(*const c_void) -> u32,
    /// WH_SF
    wh_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// DC Amps (DCA)
    ///
    /// DC Current
    dc_amps_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// DCA_SF
    dca_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// DC Voltage (DCV)
    ///
    /// DC Voltage
    dc_voltage_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// DCV_SF
    dcv_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// DC Watts (DCW)
    ///
    /// DC Power
    dc_watts_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// DCW_SF
    dcw_sf_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// Cabinet Temperature (TmpCab)
    ///
    /// Cabinet Temperature
    cabinet_temperature_callback: extern "C" fn(*const c_void) -> i16,
    /// Heat Sink Temperature (TmpSnk)
    ///
    /// Heat Sink Temperature
    heat_sink_temperature_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// Transformer Temperature (TmpTrns)
    ///
    /// Transformer Temperature
    transformer_temperature_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// Other Temperature (TmpOt)
    ///
    /// Other Temperature
    other_temperature_callback: Option<extern "C" fn(*const c_void) -> i16>,
    /// Tmp_SF
    tmp_sf_callback: extern "C" fn(*const c_void) -> i16,
    /// Operating State (St)
    ///
    /// Operating state
    operating_state_callback: extern "C" fn(*const c_void) -> Model101St,
    /// Vendor Operating State (StVnd)
    ///
    /// Vendor specific operating state code
    vendor_operating_state_callback: Option<extern "C" fn(*const c_void) -> u16>,
    /// Event1 (Evt1)
    ///
    /// Event fields
    event1_callback: extern "C" fn(*const c_void) -> u32,
    /// Event Bitfield 2 (Evt2)
    ///
    /// Reserved for future use
    event_bitfield_2_callback: extern "C" fn(*const c_void) -> u32,
    /// Vendor Event Bitfield 1 (EvtVnd1)
    ///
    /// Vendor defined events
    vendor_event_bitfield_1_callback: Option<extern "C" fn(*const c_void) -> u32>,
    /// Vendor Event Bitfield 2 (EvtVnd2)
    ///
    /// Vendor defined events
    vendor_event_bitfield_2_callback: Option<extern "C" fn(*const c_void) -> u32>,
    /// Vendor Event Bitfield 3 (EvtVnd3)
    ///
    /// Vendor defined events
    vendor_event_bitfield_3_callback: Option<extern "C" fn(*const c_void) -> u32>,
    /// Vendor Event Bitfield 4 (EvtVnd4)
    ///
    /// Vendor defined events
    vendor_event_bitfield_4_callback: Option<extern "C" fn(*const c_void) -> u32>,
}

impl ReadAdapter for Model101CallbackAdapter {
    fn amps(&self) -> u16 {
        (self.amps_callback)(self.context)
    }

    fn amps_phase_a(&self) -> u16 {
        (self.amps_phase_a_callback)(self.context)
    }

    fn amps_phase_b(&self) -> Option<u16> {
        self.amps_phase_b_callback
            .map(|callback| (callback)(self.context))
    }

    fn amps_phase_c(&self) -> Option<u16> {
        self.amps_phase_c_callback
            .map(|callback| (callback)(self.context))
    }

    fn a_sf(&self) -> i16 {
        (self.a_sf_callback)(self.context)
    }

    fn phase_voltage_ab(&self) -> Option<u16> {
        self.phase_voltage_ab_callback
            .map(|callback| (callback)(self.context))
    }

    fn phase_voltage_bc(&self) -> Option<u16> {
        self.phase_voltage_bc_callback
            .map(|callback| (callback)(self.context))
    }

    fn phase_voltage_ca(&self) -> Option<u16> {
        self.phase_voltage_ca_callback
            .map(|callback| (callback)(self.context))
    }

    fn phase_voltage_an(&self) -> u16 {
        (self.phase_voltage_an_callback)(self.context)
    }

    fn phase_voltage_bn(&self) -> Option<u16> {
        self.phase_voltage_bn_callback
            .map(|callback| (callback)(self.context))
    }

    fn phase_voltage_cn(&self) -> Option<u16> {
        self.phase_voltage_cn_callback
            .map(|callback| (callback)(self.context))
    }

    fn v_sf(&self) -> i16 {
        (self.v_sf_callback)(self.context)
    }

    fn watts(&self) -> i16 {
        (self.watts_callback)(self.context)
    }

    fn w_sf(&self) -> i16 {
        (self.w_sf_callback)(self.context)
    }

    fn hz(&self) -> u16 {
        (self.hz_callback)(self.context)
    }

    fn hz_sf(&self) -> i16 {
        (self.hz_sf_callback)(self.context)
    }

    fn va(&self) -> Option<i16> {
        self.va_callback.map(|callback| (callback)(self.context))
    }

    fn va_sf(&self) -> Option<i16> {
        self.va_sf_callback.map(|callback| (callback)(self.context))
    }

    fn v_ar(&self) -> Option<i16> {
        self.v_ar_callback.map(|callback| (callback)(self.context))
    }

    fn v_ar_sf(&self) -> Option<i16> {
        self.v_ar_sf_callback
            .map(|callback| (callback)(self.context))
    }

    fn pf(&self) -> Option<i16> {
        self.pf_callback.map(|callback| (callback)(self.context))
    }

    fn pf_sf(&self) -> Option<i16> {
        self.pf_sf_callback.map(|callback| (callback)(self.context))
    }

    fn watt_hours(&self) -> u32 {
        (self.watt_hours_callback)(self.context)
    }

    fn wh_sf(&self) -> i16 {
        (self.wh_sf_callback)(self.context)
    }

    fn dc_amps(&self) -> Option<u16> {
        self.dc_amps_callback
            .map(|callback| (callback)(self.context))
    }

    fn dca_sf(&self) -> Option<i16> {
        self.dca_sf_callback
            .map(|callback| (callback)(self.context))
    }

    fn dc_voltage(&self) -> Option<u16> {
        self.dc_voltage_callback
            .map(|callback| (callback)(self.context))
    }

    fn dcv_sf(&self) -> Option<i16> {
        self.dcv_sf_callback
            .map(|callback| (callback)(self.context))
    }

    fn dc_watts(&self) -> Option<i16> {
        self.dc_watts_callback
            .map(|callback| (callback)(self.context))
    }

    fn dcw_sf(&self) -> Option<i16> {
        self.dcw_sf_callback
            .map(|callback| (callback)(self.context))
    }

    fn cabinet_temperature(&self) -> i16 {
        (self.cabinet_temperature_callback)(self.context)
    }

    fn heat_sink_temperature(&self) -> Option<i16> {
        self.heat_sink_temperature_callback
            .map(|callback| (callback)(self.context))
    }

    fn transformer_temperature(&self) -> Option<i16> {
        self.transformer_temperature_callback
            .map(|callback| (callback)(self.context))
    }

    fn other_temperature(&self) -> Option<i16> {
        self.other_temperature_callback
            .map(|callback| (callback)(self.context))
    }

    fn tmp_sf(&self) -> i16 {
        (self.tmp_sf_callback)(self.context)
    }

    fn operating_state(&self) -> St {
        (self.operating_state_callback)(self.context)
    }

    fn vendor_operating_state(&self) -> Option<u16> {
        self.vendor_operating_state_callback
            .map(|callback| (callback)(self.context))
    }

    fn event1(&self) -> u32 {
        (self.event1_callback)(self.context)
    }

    fn event_bitfield_2(&self) -> u32 {
        (self.event_bitfield_2_callback)(self.context)
    }

    fn vendor_event_bitfield_1(&self) -> Option<u32> {
        self.vendor_event_bitfield_1_callback
            .map(|callback| (callback)(self.context))
    }

    fn vendor_event_bitfield_2(&self) -> Option<u32> {
        self.vendor_event_bitfield_2_callback
            .map(|callback| (callback)(self.context))
    }

    fn vendor_event_bitfield_3(&self) -> Option<u32> {
        self.vendor_event_bitfield_3_callback
            .map(|callback| (callback)(self.context))
    }

    fn vendor_event_bitfield_4(&self) -> Option<u32> {
        self.vendor_event_bitfield_4_callback
            .map(|callback| (callback)(self.context))
    }
}

#[repr(C)]
pub struct Model101StatefulAdapter {
    /// Amps (A)
    ///
    /// AC Current
    pub amps: u16,
    /// Amps PhaseA (AphA)
    ///
    /// Phase A Current
    ///
    /// Connected Phase
    pub amps_phase_a: u16,
    /// Amps PhaseB (AphB)
    ///
    /// Phase B Current
    pub amps_phase_b: u16,
    /// Amps PhaseC (AphC)
    ///
    /// Phase C Current
    pub amps_phase_c: u16,
    /// A_SF
    pub a_sf: i16,
    /// Phase Voltage AB (PPVphAB)
    ///
    /// Phase Voltage AB
    pub phase_voltage_ab: u16,
    /// Phase Voltage BC (PPVphBC)
    ///
    /// Phase Voltage BC
    pub phase_voltage_bc: u16,
    /// Phase Voltage CA (PPVphCA)
    ///
    /// Phase Voltage CA
    pub phase_voltage_ca: u16,
    /// Phase Voltage AN (PhVphA)
    ///
    /// Phase Voltage AN
    pub phase_voltage_an: u16,
    /// Phase Voltage BN (PhVphB)
    ///
    /// Phase Voltage BN
    pub phase_voltage_bn: u16,
    /// Phase Voltage CN (PhVphC)
    ///
    /// Phase Voltage CN
    pub phase_voltage_cn: u16,
    /// V_SF
    pub v_sf: i16,
    /// Watts (W)
    ///
    /// AC Power
    pub watts: i16,
    /// W_SF
    pub w_sf: i16,
    /// Hz (Hz)
    ///
    /// Line Frequency
    pub hz: u16,
    /// Hz_SF
    pub hz_sf: i16,
    /// VA (VA)
    ///
    /// AC Apparent Power
    pub va: i16,
    /// VA_SF
    pub va_sf: i16,
    /// VAr (VAr)
    ///
    /// AC Reactive Power
    pub v_ar: i16,
    /// VAr_SF
    pub v_ar_sf: i16,
    /// PF (PF)
    ///
    /// AC Power Factor
    pub pf: i16,
    /// PF_SF
    pub pf_sf: i16,
    /// WattHours (WH)
    ///
    /// AC Energy
    pub watt_hours: u32,
    /// WH_SF
    pub wh_sf: i16,
    /// DC Amps (DCA)
    ///
    /// DC Current
    pub dc_amps: u16,
    /// DCA_SF
    pub dca_sf: i16,
    /// DC Voltage (DCV)
    ///
    /// DC Voltage
    pub dc_voltage: u16,
    /// DCV_SF
    pub dcv_sf: i16,
    /// DC Watts (DCW)
    ///
    /// DC Power
    pub dc_watts: i16,
    /// DCW_SF
    pub dcw_sf: i16,
    /// Cabinet Temperature (TmpCab)
    ///
    /// Cabinet Temperature
    pub cabinet_temperature: i16,
    /// Heat Sink Temperature (TmpSnk)
    ///
    /// Heat Sink Temperature
    pub heat_sink_temperature: i16,
    /// Transformer Temperature (TmpTrns)
    ///
    /// Transformer Temperature
    pub transformer_temperature: i16,
    /// Other Temperature (TmpOt)
    ///
    /// Other Temperature
    pub other_temperature: i16,
    /// Tmp_SF
    pub tmp_sf: i16,
    /// Operating State (St)
    ///
    /// Operating state
    pub operating_state: Model101St,
    /// Vendor Operating State (StVnd)
    ///
    /// Vendor specific operating state code
    pub vendor_operating_state: u16,
    /// Event1 (Evt1)
    ///
    /// Event fields
    pub event1: u32,
    /// Event Bitfield 2 (Evt2)
    ///
    /// Reserved for future use
    pub event_bitfield_2: u32,
    /// Vendor Event Bitfield 1 (EvtVnd1)
    ///
    /// Vendor defined events
    pub vendor_event_bitfield_1: u32,
    /// Vendor Event Bitfield 2 (EvtVnd2)
    ///
    /// Vendor defined events
    pub vendor_event_bitfield_2: u32,
    /// Vendor Event Bitfield 3 (EvtVnd3)
    ///
    /// Vendor defined events
    pub vendor_event_bitfield_3: u32,
    /// Vendor Event Bitfield 4 (EvtVnd4)
    ///
    /// Vendor defined events
    pub vendor_event_bitfield_4: u32,
}

impl ReadAdapter for Model101StatefulAdapter {
    fn amps(&self) -> u16 {
        self.amps
    }

    fn amps_phase_a(&self) -> u16 {
        self.amps_phase_a
    }

    fn amps_phase_b(&self) -> Option<u16> {
        Some(self.amps_phase_b)
    }

    fn amps_phase_c(&self) -> Option<u16> {
        Some(self.amps_phase_c)
    }

    fn a_sf(&self) -> i16 {
        self.a_sf
    }

    fn phase_voltage_ab(&self) -> Option<u16> {
        Some(self.phase_voltage_ab)
    }

    fn phase_voltage_bc(&self) -> Option<u16> {
        Some(self.phase_voltage_bc)
    }

    fn phase_voltage_ca(&self) -> Option<u16> {
        Some(self.phase_voltage_ca)
    }

    fn phase_voltage_an(&self) -> u16 {
        self.phase_voltage_an
    }

    fn phase_voltage_bn(&self) -> Option<u16> {
        Some(self.phase_voltage_bn)
    }

    fn phase_voltage_cn(&self) -> Option<u16> {
        Some(self.phase_voltage_cn)
    }

    fn v_sf(&self) -> i16 {
        self.v_sf
    }

    fn watts(&self) -> i16 {
        self.watts
    }

    fn w_sf(&self) -> i16 {
        self.w_sf
    }

    fn hz(&self) -> u16 {
        self.hz
    }

    fn hz_sf(&self) -> i16 {
        self.hz_sf
    }

    fn va(&self) -> Option<i16> {
        Some(self.va)
    }

    fn va_sf(&self) -> Option<i16> {
        Some(self.va_sf)
    }

    fn v_ar(&self) -> Option<i16> {
        Some(self.v_ar)
    }

    fn v_ar_sf(&self) -> Option<i16> {
        Some(self.v_ar_sf)
    }

    fn pf(&self) -> Option<i16> {
        Some(self.pf)
    }

    fn pf_sf(&self) -> Option<i16> {
        Some(self.pf_sf)
    }

    fn watt_hours(&self) -> u32 {
        self.watt_hours
    }

    fn wh_sf(&self) -> i16 {
        self.wh_sf
    }

    fn dc_amps(&self) -> Option<u16> {
        Some(self.dc_amps)
    }

    fn dca_sf(&self) -> Option<i16> {
        Some(self.dca_sf)
    }

    fn dc_voltage(&self) -> Option<u16> {
        Some(self.dc_voltage)
    }

    fn dcv_sf(&self) -> Option<i16> {
        Some(self.dcv_sf)
    }

    fn dc_watts(&self) -> Option<i16> {
        Some(self.dc_watts)
    }

    fn dcw_sf(&self) -> Option<i16> {
        Some(self.dcw_sf)
    }

    fn cabinet_temperature(&self) -> i16 {
        self.cabinet_temperature
    }

    fn heat_sink_temperature(&self) -> Option<i16> {
        Some(self.heat_sink_temperature)
    }

    fn transformer_temperature(&self) -> Option<i16> {
        Some(self.transformer_temperature)
    }

    fn other_temperature(&self) -> Option<i16> {
        Some(self.other_temperature)
    }

    fn tmp_sf(&self) -> i16 {
        self.tmp_sf
    }

    fn operating_state(&self) -> St {
        self.operating_state
    }

    fn vendor_operating_state(&self) -> Option<u16> {
        Some(self.vendor_operating_state)
    }

    fn event1(&self) -> u32 {
        self.event1
    }

    fn event_bitfield_2(&self) -> u32 {
        self.event_bitfield_2
    }

    fn vendor_event_bitfield_1(&self) -> Option<u32> {
        Some(self.vendor_event_bitfield_1)
    }

    fn vendor_event_bitfield_2(&self) -> Option<u32> {
        Some(self.vendor_event_bitfield_2)
    }

    fn vendor_event_bitfield_3(&self) -> Option<u32> {
        Some(self.vendor_event_bitfield_3)
    }

    fn vendor_event_bitfield_4(&self) -> Option<u32> {
        Some(self.vendor_event_bitfield_4)
    }
}

/// C-FFI dispatch descriptor for SunSpec model 101. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_101: StaticModelSpec = StaticModelSpec {
    id: 101,
    length: model_101_c_length,
    writable: false,
    visit_read: model_101_c_visit_read,
    visit_write: model_101_c_visit_write,
};

fn model_101_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model101).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model101{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_101_c_visit_read(
    kind: u8,
    adapter: *const c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    cursor: &mut Cursor<ModbusException>,
    buffer: &mut WritableRegisterBuffer<'_>,
) {
    let model = Model101;
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model101StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model101CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
    cursor.visit_source_block(model.model_length(), |offset, from, len| {
        let mut block = buffer.slice(from, len);
        match adapter {
            Some(adapter) => model.traverse_points_read(adapter, &mut block, offset),
            None => {
                block.fill(&[0xff, 0xff]);
                Ok(())
            }
        }
    });
}

/// # Safety
/// As for [`model_101_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_101_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    cursor: &mut Cursor<ModbusException>,
    buffer: &ReadableRegisterBuffer<'_>,
) {
    let _ = (kind, adapter, buffer);
    let model = Model101;
    cursor.visit_source_block(model.model_length(), |_, _, _| {
        Err(ModbusException::IllegalDataAddress)
    });
}
