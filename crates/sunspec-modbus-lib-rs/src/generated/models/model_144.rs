...
/// Marker for SunSpec model 144. Add it to a [`Sunspec`](crate::Sunspec) model list.
pub struct Model144;

impl<'ad> ModelSpec<'ad> for Model144 {
    const MODEL_ID: u16 = 144;
...
impl WriteAdapter for Model144CallbackAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
impl WriteAdapter for Model144StatefulAdapter {
    /// ActCrv (ActCrv)
    ///
    /// Index of active curve. 0=no active curve.
    fn set_act_crv(&mut self, value: u16) {
...
/// C-FFI dispatch descriptor for SunSpec model 144. A C `SunspecModelBinding` points
/// at this static, so the service functions dispatch with no model-id lookup.
#[unsafe(no_mangle)]
pub static SUNSPEC_MODEL_144: StaticModelSpec = StaticModelSpec {
    id: 144,
    length: model_144_c_length,
    writable: true,
    visit_read: model_144_c_visit_read,
    visit_write: model_144_c_visit_write,
};

fn model_144_c_length(_repeat_count_0: u16, _repeat_count_1: u16) -> u16 {
    (Model144).model_length()
}

/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model144{Stateful,Callback}Adapter`,
/// valid for the duration of the call.
unsafe fn model_144_c_visit_read(
    kind: u8,
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model144StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model144CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
...
/// # Safety
/// As for [`model_144_c_visit_read`], and `adapter` must be uniquely borrowable for the call.
unsafe fn model_144_c_visit_write(
    kind: u8,
    adapter: *mut c_void,
    _repeat_count_0: u16,
    _repeat_count_1: u16,
    ...
    let adapter: Option<&mut dyn WriteAdapter> = match kind {
        1 => {
            Some(unsafe { &mut *(adapter as *mut Model144StatefulAdapter) } as &mut dyn WriteAdapter)
        }
        2 => {
            Some(unsafe { &mut *(adapter as *mut Model144CallbackAdapter) } as &mut dyn WriteAdapter)
        }
        _ => None,
    };
