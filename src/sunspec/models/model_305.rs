...
pub fn write_point(model: &dyn ModelAdapter, point: &Point, buffer: &mut [u16], offset: u16, limit: u16) {
    match point {
        Point::Tm => if let Some(value) = model.tm() { serialisation::write_string(value, buffer, offset, limit); },
        Point::Date => if let Some(value) = model.date() { serialisation::write_string(value, buffer, offset, limit); },
        Point::Location => if let Some(value) = model.location() { serialisation::write_string(value, buffer, offset, limit); },
        Point::Lat => if let Some(value) = model.lat() { serialisation::write_i32(value, buffer, offset, limit); },
        Point::Long => if let Some(value) = model.long() { serialisation::write_i32(value, buffer, offset, limit); },
        Point::Altitude => if let Some(value) = model.altitude() { serialisation::write_i32(value, buffer, offset, limit); },
    }
}
...
pub trait ModelAdapter {
    ...
    fn location(&self) -> Option<String<40>> {
        None
    }
