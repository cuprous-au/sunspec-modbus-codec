...
use sunspec_modbus_lib_rs::sunspec::models::model_141::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model141CallbackAdapter`, valid for the
...
unsafe fn model_141_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model141CallbackAdapter).as_ref() };
...
unsafe fn model_141_c_visit_write(
    ...
    let model = Model141;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model141CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_141_callback(
    adapter: *mut Model141CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_141,
