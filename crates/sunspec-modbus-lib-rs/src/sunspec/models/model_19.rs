...
pub fn write_point<'a>(
    ...
    match point {
        ...
        Point::Rate => {
            buffer::write_u32(model.rate(), buffer, offset, limit);
        }
        Point::Bits => {
            buffer::write_u16(model.bits(), buffer);
        }
        Point::Parity => {
            buffer::write_u16(model.parity() as u16, buffer);
        }
        Point::Duplex => {
