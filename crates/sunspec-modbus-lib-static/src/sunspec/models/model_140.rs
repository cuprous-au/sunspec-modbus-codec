...
use sunspec_modbus_lib_rs::sunspec::models::model_140::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model140CallbackAdapter`, valid for the
...
unsafe fn model_140_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model140CallbackAdapter).as_ref() };
...
unsafe fn model_140_c_visit_write(
    ...
    let model = Model140;
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model140CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_140_callback(
    adapter: *mut Model140CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_140,
