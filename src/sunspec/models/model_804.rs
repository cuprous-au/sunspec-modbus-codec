...
pub fn write_point(model: &dyn ModelAdapter, point: &Point, buffer: &mut [u16], offset: u16, limit: u16) {
    match point {
        Point::StringIndex => {
            serialisation::write_u16(model.string_index(), buffer);
        },
        Point::ModuleCount => {
            serialisation::write_u16(model.module_count(), buffer);
        },
        Point::StringStatus => {
        ...
        Point::StringCurrent => {
            serialisation::write_i16(model.string_current(), buffer);
        },
        Point::StringVoltage => {
        ...
        Point::MaxCellVoltage => {
            serialisation::write_u16(model.max_cell_voltage(), buffer);
        },
        ...
        Point::MinCellVoltage => {
            serialisation::write_u16(model.min_cell_voltage(), buffer);
        },
        ...
        Point::AverageCellVoltage => {
            serialisation::write_u16(model.average_cell_voltage(), buffer);
        },
        ...
        Point::SoCSf => {
            serialisation::write_u16(model.so_c_sf(), buffer);
        },
        Point::SoHSf => {
        ...
        Point::CellVSf => {
            serialisation::write_u16(model.cell_v_sf(), buffer);
        },
        Point::ModTmpSf => {
            serialisation::write_u16(model.mod_tmp_sf(), buffer);
        },
    }
}
...
impl ModelAdapter for Model804CallbackAdapter {
    ...
    /// Connection Failure Reason
    fn connection_failure_reason(&self) -> Option<ConFail> {
        self.connection_failure_reason_callback.map(|callback| {
    ...
    /// Number of cells currently being balanced in the string.
    fn string_cell_balancing_count(&self) -> Option<u16> {
        self.string_cell_balancing_count_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Measurement.
    fn string_depth_of_discharge(&self) -> Option<u16> {
        self.string_depth_of_discharge_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Number of discharge cycles executed upon the string.
    fn string_cycle_count(&self) -> Option<u32> {
        self.string_cycle_count_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Measurement.
    fn string_state_of_health(&self) -> Option<u16> {
        self.string_state_of_health_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Measurement.
    fn string_voltage(&self) -> Option<u16> {
        self.string_voltage_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Status of the contactor(s) for the string.
    fn contactor_status(&self) -> Option<u32> {
        self.contactor_status_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Reserved for future use.
    fn string_event_2(&self) -> Option<u32> {
        self.string_event_2_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Enables and disables the string. Should reset to 0 upon completion.
    fn enable_disable_string(&self) -> Option<u16> {
        self.enable_disable_string_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Should reset to 0 upon completion.
    fn connect_disconnect_string(&self) -> Option<SetCon> {
        self.connect_disconnect_string_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Scale factor for string state of health.
    fn so_h_sf(&self) -> Option<u16> {
        self.so_h_sf_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Scale factor for string depth of discharge.
    fn do_d_sf(&self) -> Option<u16> {
        self.do_d_sf_callback.map(|callback| {
        (callback)(self.context)
        })
    }
    ...
    /// Scale factor for string voltage.
    fn v_sf(&self) -> Option<u16> {
        self.v_sf_callback.map(|callback| {
        (callback)(self.context)
        })
    }
...
impl ModelAdapter for Model804StatefulAdapter {
    ...
    fn string_cell_balancing_count(&self) -> Option<u16> {
        Some(
        self.string_cell_balancing_count
        )
    }
    ...
    fn string_depth_of_discharge(&self) -> Option<u16> {
        Some(
        self.string_depth_of_discharge
        )
    }
    ...
    fn connect_disconnect_string(&self) -> Option<SetCon> {
        Some(
        self.connect_disconnect_string
        )
    }
