...
impl ReadAdapter for Model201StatefulAdapter {
    fn amps(&self) -> i16 {
        self.amps
    }
...
/// # Safety
/// For `kind` 1 or 2, `adapter` must point to a live `Model201{Stateful,Callback}Adapter`,
...
unsafe fn model_201_c_visit_read(
    ...
    let adapter: Option<&dyn ReadAdapter> = match kind {
        1 => Some(unsafe { &*(adapter as *const Model201StatefulAdapter) } as &dyn ReadAdapter),
        2 => Some(unsafe { &*(adapter as *const Model201CallbackAdapter) } as &dyn ReadAdapter),
        _ => None,
    };
