...
use sunspec_modbus_lib_rs::sunspec::models::model_709::*;
...
/// # Safety
/// `adapter` must be null or point to a live `Model709CallbackAdapter`, valid for the
...
unsafe fn model_709_c_visit_read(
    ...
    let adapter = unsafe { (adapter as *const Model709CallbackAdapter).as_ref() };
...
unsafe fn model_709_c_visit_write(
    ...
    // SAFETY: as required by this function's own contract.
    let adapter = unsafe { (adapter as *mut Model709CallbackAdapter).as_mut() };
...
pub extern "C" fn sunspec_model_709_callback(
    adapter: *mut Model709CallbackAdapter,
) -> SunspecAdapter {
    SunspecAdapter {
        model_spec: &SUNSPEC_MODEL_709,
